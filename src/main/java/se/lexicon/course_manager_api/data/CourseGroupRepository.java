...
public interface CourseGroupRepository extends JpaRepository<CourseGroup, String> {
    @Query("SELECT c FROM CourseGroup c WHERE c.startDate > current_date")
    List<CourseGroup> findNotYetStarted();

    @Query("SELECT c FROM CourseGroup c WHERE c.startDate > current_date AND c.course.id = :id")
    List<CourseGroup> findNotYetStarted(@Param("id") String courseId);

    @Query("SELECT c FROM CourseGroup c WHERE current_date BETWEEN c.startDate AND c.endDate")
    List<CourseGroup> findCurrentCourseGroups();

    @Query("SELECT c FROM CourseGroup c WHERE :date BETWEEN c.startDate AND c.endDate")
    List<CourseGroup> findCourseGroupsBy(@Param("date") LocalDate date);

    @Query("SELECT c FROM CourseGroup c WHERE :date > c.endDate")
    List<CourseGroup> findByEndDateAfter(@Param("date") LocalDate date);

    @Query("SELECT c FROM CourseGroup c WHERE c.course.id = :id")
